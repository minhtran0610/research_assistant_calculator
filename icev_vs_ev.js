...
getICEVInformation(
    inputMake_1, inputModel_1, inputVersion_1, inputYear_1, carMakeOptions_1, carModelOptions_1, carVersionOptions_1, carYearOptions_1, averageEmissionElement
);
getEVInformation(inputMake_2, inputModel_2, carMakeOptions_2, carModelOptions_2, efficiencyElement);
...
getDistanceTravelTimeAndTemperatureICEV(inputOrigin_1, inputDestination_1, distanceElement_1, travelTimeElement, temperatureElement_1);
getDistanceAndTemperatureEV(inputOrigin_2, inputDestination_2, distanceElement_2, temperatureElement_2);
...
okButton.onclick = () => {
    ...
    let temperature_1 = parseFloat(temperatureElement_1.textContent.slice(41,45));

    let efficiency = parseFloat(efficiencyElement.textContent);
    let distance_2 = parseFloat(distanceElement_2.textContent);
    let temperature_2 = parseFloat(temperatureElement_2.textContent.slice(41,45));
    ...
    let totalEmission_1 = calculateICEVEmission(averageEmission, distance_1, temperature_1, travelTime, totalEmissionElement_1);
    // console.log(efficiency, distance_2, temperature_2, emissionFactor);
    let totalEmission_2 = calculateEVEmission(efficiency, distance_2, temperature_2, emissionFactor, totalEmissionElement_2);

    // Compare the results
    checkSmallerValue(totalEmissionElement_1, totalEmissionElement_2, totalEmission_1, totalEmission_2);
}
...
async function getICEVInformation(inputMake, inputModel, inputVersion, inputYear, carMakeOptions, carModelOptions, carVersionOptions, carYearOptions, emissionElement) {
    // Fetch the database
    console.log('Fetching....');
    const carDatabase = await fetchCarData('https://minhtran0610.github.io/data/car_data_2.json');
...
async function getEVInformation(inputMake, inputModel, carMakerOptions, carModelOptions, efficiencyElement) {
    // Fetch the data
    console.log('Fetching....');
    const carDatabase = await fetchCarData('https://minhtran0610.github.io/data/ev_data.json');
...
async function getDistanceTravelTimeAndTemperatureICEV(inputOrigin, inputDestination, distanceResult, travelTimeResult, temperatureTextResult) {
...
async function getDistanceAndTemperatureEV(inputOrigin, inputDestination, distanceResult, temperatureTextResult) {
    inputOrigin.onchange = () => {
        inputDestination.disabled = false;
    }
...
function getEfficiencyPercentage(temperature, environmentEffect) {
    ...
    closestDataPoint = findClosest(intDataPoints, temperature);
    return Math.round(1000000/environmentEffect.temperature[closestDataPoint])/100;
}
...
function calculateICEVEmission(averageEmission, distance, temperature, travelTime, totalEmissionElement) {
    ...
    let increaseInTemperature = getIncreaseInTemperature(temperature, ICEV_ENVIRONMENT_EFFECT);
    let acFuelUsage = getAcFuelUsage(temperature, ICEV_ENVIRONMENT_EFFECT);
    totalEmission = Math.round((totalEmission * (1+increaseInTemperature) + 2350 * acFuelUsage * travelTime)*100)/100;
...
function calculateEVEmission(efficiency, distance, temperature, emissionFactor, totalEmissionElement) {
    ...
    let efficiencyPercentage = getEfficiencyPercentage(temperature, EV_ENVIRONMENT_EFFECT);
